...
import kosta.web.mvc.model.user.UserMapperRef;

/**
 * DB, DAO관련 설정들 - 사용할 mapper, dto주소 등록 필요
 *
 */
@Configuration
@EnableTransactionManagement //TransactionManagementConfigurer를 구현한 클래스에 필수선언(상속가능위해)
...
public class PersistenceConfig  implements TransactionManagementConfigurer{//<tx:annotation-driven
	...
	//MyBatis말고 JPA를 쓰더라도 이 환경설정의 틀을 그대로 사용할 수 있다. 
	//->dbInfo, mapper정보들, alias 정도만 거기에맞게 수정
	//경로 설정 : 위의 @Property Source 주소, mapper(xml이나 interface기반), aliaspackage(DTO)
	...
	@Value("${db.userName}") //설정후아래에서 사용
	private String username;
	
	@Value("${db.userPass}") //설정후아래에서 사용
	 ...
	 * 즉, Ant-style을 사용하기 위해서 위의 리졸버가 필요
	 * ->근데 일단 사용안해서 주석처리해놓음
	 * */
	@Bean
...
//		factoryBean.setMapperLocations(resource);
		factoryBean.setTypeAliasesPackage("kosta.web.mvc.model.dto");
	 ...
	 * */
	@Bean
	public MapperFactoryBean<UserMapperRef> getUserMapper() throws Exception{
		System.out.println("getUserMapper()......................");
		MapperFactoryBean<UserMapperRef> userMapper = new MapperFactoryBean<UserMapperRef>();
		userMapper.setMapperInterface(UserMapperRef.class);
